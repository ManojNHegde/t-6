...
def generate_branch_name(
    ...
    # Extract branch name if the response contains conversational text
    # Look for lines that start with feature/bug/chore and contain the ADW ID
    if "\n" in branch_name or not (branch_name.startswith("feature-") or branch_name.startswith("bug-") or branch_name.startswith("chore-")):
        lines = branch_name.split("\n")
        for line in lines:
            line = line.strip()
            # Look for branch name pattern: type-issue-N-adw-ID-description
            if (line.startswith("feature-") or line.startswith("bug-") or line.startswith("chore-")) and adw_id in line:
                branch_name = line
                logger.info(f"Extracted branch name from output: {branch_name}")
                break
