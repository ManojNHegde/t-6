...
def main():
    ...
    # Extract file path if the response contains conversational text
    # Look for lines that start with "specs/" or contain ".md"
    if "\n" in plan_file_path or not plan_file_path.startswith("specs/"):
        lines = plan_file_path.split("\n")
        for line in lines:
            line = line.strip()
            if line.startswith("specs/") and line.endswith(".md"):
                plan_file_path = line
                logger.info(f"Extracted plan file path from output: {plan_file_path}")
                break
