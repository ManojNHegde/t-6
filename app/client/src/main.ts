...
function displayResults(response: QueryResponse, query: string) {
  ...
  // Add download button to results header if not already present
  const resultsHeader = document.querySelector('.results-header') as HTMLElement;
  if (!resultsHeader.querySelector('.download-results-button')) {
    ...
    downloadButton.onclick = async () => {
      if (currentQueryResults && !currentQueryResults.error) {
        try {
          await api.exportQueryResults(currentQueryResults);
        } catch (error) {
...
function displayTables(tables: TableSchema[]) {
  ...
  tables.forEach(table => {
    ...
    // Add download button before remove button
    const downloadButton = document.createElement('button');
