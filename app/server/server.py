...
async def export_table(table_name: str):
    ...
    try:
        ...
        # Query entire table using execute_sql_safely (not execute_query_safely as per spec)
        sql = f"SELECT * FROM {table_name}"
        result = execute_sql_safely(sql)

        if result['error']:
            raise HTTPException(500, f"Error querying table: {result['error']}")

        # Generate CSV using csv.DictWriter
        output = io.StringIO()
        ...
        logger.info(f"[SUCCESS] Table exported: {table_name}, rows: {len(result['results'])}")
        return response
...
async def export_query_results(request: ExportQueryResultsRequest):
    ...
    try:
        ...
        if request.results:
            ...
            # No results - write headers only
            writer = csv.DictWriter(output, fieldnames=request.columns if request.columns else ['no_data'])
            writer.writeheader()
